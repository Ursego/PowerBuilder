...
// The reason is that many developers don't attach importance to working with constants correctly, but this could make the work easier and reduce errors.
...
// This naming convention offers two key benefits:

// • It helps readers clearly distinguish between the two parts, even when both parts contain multiple words.
...
// Constants that are used globally must be declared in non-autoinstantiated NVOs — one NVO for each constants group (or "family").
...
// For example, if you create a class named n_order_status and look at its source, you'll see:

global n_order_status n_order_status

// That allows you to access constants just through the class name (which is really an object name) - exactly like you would access static fields in C# or Java:
...
// This helps developers see the full picture for that family when they look at the declaration section, and if they need one in the future, it's already there.  
...
// If you need to pass an order status to a function, the argument type will still be string, not n_order_status - i.e. not strongly-typed.
...
// If a constant is defined in the application for a value you're using, ALWAYS use the constant instead of hard-coding.
...
// Also, it facilitates debugging - it's easy to search for n_order_status.OPEN globally, but if it's "OPN" somewhere, you won't find the spot.
...
// For example, enstead of
ls_expr = "order_status='OPN'"
// write:
ls_expr = "order_status='" + n_order_status.OPEN + "'"
...
//     for the respective values — instead of hard-coding them in the DataWindow Painter (BTW, did you know that "painter" comes from "paint", not from "pain" :-D ?).
// This can be done using the SetValue() method of the DataWindow.

// For example, if you need to create a radio button for the field payment_method, and the constants n_payment_method.CREDIT_CARD (with value "C") and
// n_payment_method.PAYPAL (with value "P") already exist, use those constants directly instead of hard-coded values:
...
// Have a ready-to-use set of null values for different data types, instead of calling SetNull() each time you need a null variable.  
...
// Sometimes we use variables that are set to null — for example, to pass a null to a function, or, conversely, return a null from a function.
// Developers usually declare a variable and nullify it using SetNull() in the same script where it's used.
// But these two lines (really, hundreds of lines across the project) can be avoided by declaring null constants for different data types in a globally accessible object.

// Technically, PowerBuilder doesn't allow initializing constants with NULL.
// Instead, we will declare a set of public instance variables using privatewrite, and set them to NULL in the constructor.
// Physically, they are variables, but they effectively behave like constants, so we use the constants naming convention.
