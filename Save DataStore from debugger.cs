...
// For that, insert a new Watch with one of the following expressions (change ids_XXX to your real DS, and customize the file path):

ids_XXX.SaveAs("C:\dev\misc\ds_debug\" + String(Today(), "yyyymmdd") + "_" + String(Now(), "hhmmss") + "_" + ids_XXX.DataObject + ".xls", Excel!, true)
ids_XXX.SaveAs("C:\dev\misc\ds_debug\" + String(Today(), "yyyymmdd") + "_" + String(Now(), "hhmmss") + "_" + ids_XXX.DataObject + ".txt", Text!, true)

// Data will be saved:
// 1. Immediately after you have created the Watch.
// 2. Each time the script is executed in the debug mode.
...
// That also allows you to compare versions of data before and after a fix.
